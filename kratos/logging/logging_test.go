...
func TestExtractArgs_Nil(t *testing.T) {
	...
	raw, ok := result.(json.RawMessage)
	if !ok {
		t.Fatalf("Expected json.RawMessage for nil, got %T", result)
	}
	if string(raw) != "{}" {
...
func TestServer_Success(t *testing.T) {
	...
	if !strings.Contains(logOutput, `"msg":"server"`) {
		t.Errorf("Expected server log message, got: %s", logOutput)
	}
...
func TestClient_Success(t *testing.T) {
	...
	logOutput := buf.String()
	if !strings.Contains(logOutput, `"msg":"client"`) {
		t.Errorf("Expected client log message, got: %s", logOutput)
	}
